        ...
        match resp.status() {
            StatusCode::OK => {
                ...
                let cst = match resp.headers().get("CST") {
                    Some(value) => {
                        let cst_str = value.to_str().map_err(|_| AuthError::Unexpected(StatusCode::OK))?;
                        tracing::info!("Successfully obtained CST token of length: {}", cst_str.len());
                        cst_str.to_owned()
                    },
                    None => {
                ...
                let token = match resp.headers().get("X-SECURITY-TOKEN") {
                    Some(value) => {
                        let token_str = value.to_str().map_err(|_| AuthError::Unexpected(StatusCode::OK))?;
                        tracing::info!("Successfully obtained X-SECURITY-TOKEN of length: {}", token_str.len());
                        token_str.to_owned()
                    },
                    None => {
            ...
            StatusCode::UNAUTHORIZED => {
                ...
                let body = resp.text().await.unwrap_or_else(|_| "Could not read response body".to_string());
        ...
        match resp.status() {
            StatusCode::OK => {
                ...
                let cst = match resp.headers().get("CST") {
                    Some(value) => {
                        let cst_str = value.to_str().map_err(|_| AuthError::Unexpected(StatusCode::OK))?;
                        tracing::info!("Successfully obtained refreshed CST token of length: {}", cst_str.len());
                        cst_str.to_owned()
                    },
                    None => {
                ...
                let token = match resp.headers().get("X-SECURITY-TOKEN") {
                    Some(value) => {
                        let token_str = value.to_str().map_err(|_| AuthError::Unexpected(StatusCode::OK))?;
                        tracing::info!("Successfully obtained refreshed X-SECURITY-TOKEN of length: {}", token_str.len());
                        token_str.to_owned()
                    },
                    None => {
