...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let epics = match epics_arg {
        Some(arg) => {
            ...
            arg.split(',').map(|s| s.trim().to_string()).collect::<Vec<String>>()
        },
        None => {
            // Generate the default range of EPICs
            info!("No EPICs provided, using default range from DO.D.OTCDDAX.1.IP to DO.D.OTCDDAX.196.IP");
            (1..=196).map(|i| format!("DO.D.OTCDDAX.{}.IP", i)).collect::<Vec<String>>()
        }
    };
    ...
    for chunk_start in (0..epics.len()).step_by(BATCH_SIZE) {
        ...
        info!("Fetching market details for batch {}/{} (EPICs {}-{} of {})", 
        ...
        match market_service.get_multiple_market_details(&session, epics_chunk).await {
            Ok(details_vec) => {
                ...
                info!("✅ Successfully processed batch of {} EPICs ({}/{})", 
                      details_vec.len(), processed_count, total_epics);
            },
            Err(e) => {
        ...
        if processed_count > 0 {
            info!("Processed {}/{} EPICs. Saving intermediate results...", processed_count, epics.len());
    ...
    println!("{:-<40} {:-<15} {:-<10} {:-<10} {:-<10} {:-<10} {:-<20} {:-<15}", 
        "", "", "", "", "", "", "", "");
    ...
    for (epic, details) in &market_details_vec {
        ...
        let high_low = format!("{}/{}",
            details.snapshot.high.map(|h| h.to_string()).unwrap_or_else(|| "-".to_string()),
            details.snapshot.low.map(|l| l.to_string()).unwrap_or_else(|| "-".to_string())
        );
        
        // Get the last dealing date from expiry_details if available
        let last_dealing_date = details.instrument.expiry_details.as_ref()
        ...
        println!(
            ...
            truncate(epic, 13),
            details.snapshot.bid.map(|b| b.to_string()).unwrap_or_else(|| "-".to_string()),
            details.snapshot.offer.map(|o| o.to_string()).unwrap_or_else(|| "-".to_string()),
    ...
    info!("Successfully processed {} out of {} EPICs", market_details_vec.len(), epics.len());
